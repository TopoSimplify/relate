...
//Direction Relate
func QuadRelate(polyline *pln.Polyline, contexts *ctx.ContextGeometries) []string {
	...
	var quadRelations []string
	var lnbox = polyline.BBox()

	for i, n := 0, contexts.Len();  i < n; i++ {
		g = geometries[i]
		var gbox = g.BBox()
		...
		quadRelations = append(quadRelations, strings.Join(quads, ""))
	}
