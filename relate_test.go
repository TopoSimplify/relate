...
func TestRelate(t *testing.T) {
	...
	g.Describe("test relate", func() {
		g.It("should test relate", func() {
			...
			for _, h := range hulls {
				g.Assert(IsGeomRelateValid(insDP, h, const_geom)).IsTrue()
				g.Assert(IsDirRelateValid(insDP, h, const_geom)).IsTrue()
				g.Assert(IsDistRelateValid(insDP, h, const_geom)).IsTrue()
			}
			...
			const_geom = ctx.NewCtxGeom(neib, 0, -1).AsContextNeighbour()
			g.Assert(IsGeomRelateValid(insDP, hulls[0], const_geom)).IsFalse()
			g.Assert(IsGeomRelateValid(insDP, hulls[1], const_geom)).IsTrue()
			g.Assert(IsGeomRelateValid(insDP, hulls[2], const_geom)).IsTrue()
			...
			g.Assert(IsGeomRelateValid(insDP,hulls[0], const_geom)).IsFalse()
			g.Assert(IsGeomRelateValid(insDP,hulls[1], const_geom)).IsTrue()
			g.Assert(IsGeomRelateValid(insDP,hulls[2], const_geom)).IsFalse()

			g.Assert(IsDistRelateValid(insDP, hulls[0], const_geom)).IsFalse()
			g.Assert(IsDistRelateValid(insDP, hulls[1], const_geom)).IsTrue()
			g.Assert(IsDistRelateValid(insDP, hulls[2], const_geom)).IsFalse()
		})
	})
}
